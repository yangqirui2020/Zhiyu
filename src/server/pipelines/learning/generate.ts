...
export async function prepareLearning(noteText: string, classroom: Classroom, provider: StructuredOutputProvider, context: ExecutionContext) {
  ...
  const generated = await provider.generate({
    ...
    system: untrusted + (repair ? repairInstruction : "") + "从给定学生中选择一位与用户观点有可解释关系的同桌，说明共同点、差异和匹配理由。仅生成一次具体追问，澄清用户尚未给出的条件或检验方法。使用系统视角提问，不使用答主第一人称。sampleAnswer 是供用户编辑的第一人称直接参考回应，必须回答刚才那一次追问。不要描述如何生成追问、不要写“可以这样回应”，不能称为用户已经说过。evidenceIds 至少含这位学生所属来源的证据。",
