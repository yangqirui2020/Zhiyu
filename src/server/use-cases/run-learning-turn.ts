...
async function execute(request: LearningTurnRequest, context: ExecutionContext): Promise<ApiSuccess<LearningTurnResult>> {
  ...
  if (exactNote && (request.stage === "prepare" || exactCompletion)) {
    ...
    return { ok: true, data, meta: { requestId: context.requestId, mode: "sample", servedAt: new Date().toISOString(), snapshotId: manifest.snapshotId, capturedAt: manifest.capturedAt, warnings: ["当前结果与示例观点、追问及本阶段输入精确匹配，是预计算示例。"] } };
  }
  const data = await new LiveLearningTurnProvider().run(request, context);
  return { ok: true, data, meta: { requestId: context.requestId, mode: "live", servedAt: new Date().toISOString(), warnings: ["根据你的输入实时整理；AI 草稿请自行核对。"] } };
}
