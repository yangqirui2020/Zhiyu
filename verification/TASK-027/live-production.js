...
async () => {
  ...
  const request = async (name, path, body) => {
    ...
    const response = await fetch(path, { ...(body ? { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) } : {}), signal: AbortSignal.timeout(35000) });
  ...
  const candidate = await request("live_candidate", "/api/v1/candidate-seat", { ...base, idempotencyKey: `task027_candidate_${Date.now()}` });
  const prepared = await request("live_prepare", "/api/v1/learning-turn", { ...base, stage: "prepare", idempotencyKey: `task027_prepare_${Date.now()}` });
  const complete = await request("live_complete", "/api/v1/learning-turn", { ...base, stage: "complete", answerText, challengeToken: prepared.data.challengeToken, idempotencyKey: `task027_complete_${Date.now()}` });
  if ([candidate, prepared, complete].some(item => item.meta.mode !== "live")) throw new Error("Non-sample input did not use Live");
  ...
  return { testedAt: new Date().toISOString(), origin: location.origin, syntheticQaInput: true, classroomMode: classroom.provenance.mode, sourceCount: classroom.sources.length, results, inputPreserved: true, evidenceInCurrentClassroom: true, candidateDecision: candidate.data.assessments[0].decision };
}
